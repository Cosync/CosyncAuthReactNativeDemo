...
import CosyncAuth from 'cosync-auth-react-native';  
...
export function AuthProvider({ children }) {
    ...
    const cosyncAuth = new CosyncAuth(Configure.CosyncApp).getInstance();
    ...
    async function getApplication() {
        ...
        cosyncAuth.app.getApplication().then(result => {  
            setAppData(result) 
    ...
    async function loginAnonymous(){
        ...
        try {
            ...
            let id =  uuid.v4();
            let result = await cosyncAuth.login.loginAnonymous(`ANON_${id}`); 
    ...
    async function login(userEmail, userPassword) { 
        try {
            ...
            let result = await cosyncAuth.login.login(userEmail, userPassword)
            if(result.jwt){
                loginJWT(result);
            }
    ...
    async function loginComplete(loginCode){
        try {
            let result = await cosyncAuth.login.loginComplete(loginToken, loginCode);
            if(result.jwt){
                loginJWT(result);
            }
    ...
    async function signup(userEmail, userPassword, metaData, locale){
        try {
            let result = await cosyncAuth.signup.signup(userEmail, userPassword, metaData, locale); 
    ...
    async function signupComplete(userEmail, signupCode){
        try {
            let result = await cosyncAuth.signup.completeSignup(userEmail, signupCode);
    ...
    async function loginJWT(tokenData){
        ...
        let user = await cosyncAuth.realmManager.login(tokenData.jwt, Configure.Realm.appId); 
        setRealmUser(user)
        console.log("get realm user ", user.id); 
        let data = await cosyncAuth.profile.getUser();
        setUserData(data);
    ...
    async function register(userEmail, userPassword, inviteCode, metaData, locale){
        try {
            let result = await cosyncAuth.register.register(userEmail, userPassword, inviteCode, metaData, locale);
            if(result && result.jwt){ 
                loginJWT(result)
            }
