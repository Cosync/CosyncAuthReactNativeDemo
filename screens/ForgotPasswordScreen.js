...
const ForgotPasswordScreen = props => { 
  ...
  const { getApplication, appData, cosyncAuth } = useContext(AuthContext)
  ...
  const handleResetPassword = () => {
    ...
    if(verifyCode){
      ...
      setLoading(true);  
      cosyncAuth.password.forgotPassword(userEmail).then(result => {

        setLoading(false);
        console.log('CosyncAuth forgotPassword result  ', result);
        ...
        setLoading(false);
        console.log('CosyncAuth forgotPassword err  ', err);
  ...
  const handleSubmitVerifyCodePress = () => {
    ...
    let validate = cosyncAuth.password.validatePassword(userPassword);
    if(!validate){
      let message = `
      ...
      cosyncAuth.password.resetPassword(userEmail, userPassword, resetCode).then(result => {
        setLoading(false); 
