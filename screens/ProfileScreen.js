...
const ProfileScreen = props => { 
  ...
  const { userData, getApplication, cosyncAuth, appData, appLocales } = useContext(AuthContext);
  ...
  const handleInvite = () => { 
    ...
    cosyncAuth.profile.invite(userEmail, metadata).then(result => { 
  ...
  const updatePhoneTwoFactor = () => {  
    ...
    cosyncAuth.profile.setTwoFactorGoogleVerification(isTwoFactor).then(result => {  
  ...
  const handleAddPhone = () => {  
    ...
    if(isVerifyPhone){  
      ...
      cosyncAuth.profile.verifyPhone(userPhoneCode).then(result => { 
    ...
    cosyncAuth.profile.setPhone(userPhone).then(result => { 
  ...
  const setUserLocale = (locale) => {
    ...
    cosyncAuth.profile.setLocale(locale).then(result => { 
  ...
  const handleUpdateProfile = () => {
    ...
    cosyncAuth.profile.setUserMetadata(metadata).then(result => { 
      if(result == true){
        alert('Success');
      } 
      else{ 
