...
const PasswordScreen = props => { 
  ...
  const { logout, getApplication, cosyncAuth, appData } = useContext(AuthContext);
  ...
  const handleResetPassword = () => {
    ...
    let validate = cosyncAuth.password.validatePassword(newUserPassword);
    if(!validate){
      let message = `
      ...
      cosyncAuth.password.changePassword(userPassword, newUserPassword).then(result => {

        setUserPassword(' ');
        setNewUserPassword(' ');

        setLoading(false);
        console.log('CosyncAuth PasswordScreen result  ', result);
        ...
        setLoading(false);
        console.log('CosyncAuth PasswordScreen err  ', err);
